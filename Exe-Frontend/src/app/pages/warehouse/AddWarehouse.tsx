...
/** Inline SVG pin icon — no PNG imports, works in all bundlers */
...
function parseAddress(addr: NominatimAddress, displayName = "") {
  ...
  const road =
    addr.road || addr.pedestrian || addr.footway || addr.path || addr.place || "";
  const district =
    addr.city_district || addr.county || addr.town || addr.suburb ||
    addr.quarter || addr.neighbourhood || addr.village || addr.hamlet || "";
...
function matchProvince(prov: string): string {
  ...
  return (
    ...
    vietnamProvinces.find((p) =>
      normalize(p).includes(needle) || needle.includes(normalize(p)),
    ) ||
    ""
  );
}
...
// ── File types ─────────────────────────────────────────────────────────────
interface CertFile { name: string; size: number; file: File; }
...
export default function AddWarehouse() {
  ...
  const quickFill = () => {
    setFormData({
      name: "Kho lạnh Cát Lái Test",
      description: "Kho lạnh kiểm thử với hệ thống làm lạnh 3 tầng, trang bị máy phát điện dự phòng 500KVA và hệ thống CCTV 24/7.",
      houseNumber: "123",
      address: "Đường Nguyễn Thị Định",
      city: "Quận 2",
    ...
    setSections([
      {
        ...
        description: "Hải sản, thịt đông lạnh",
        capacity: "2000",
        availableCapacity: "1500",
        temperatureMin: "-25",
        temperatureMax: "-18",
        priceTiers: [{ id: "tier-qf-1", label: "Giá thuê tháng", value: "320000", unit: "month" }],
        availability: "available",
        collapsed: false,
      },
      {
        id: `sec-qf-2`,
        name: "Phòng mát",
        description: "Rau củ quả, sữa",
        capacity: "1500",
        availableCapacity: "800",
        temperatureMin: "2",
        temperatureMax: "8",
        priceTiers: [{ id: "tier-qf-2", label: "Giá thuê tháng", value: "180000", unit: "month" }],
        availability: "partially",
        collapsed: false,
      },
    ]);
    setWarehouseTiers([
      { id: "tier-wh-basic", label: "Gói Cơ bản", value: "250000", unit: "month" },
      { id: "tier-wh-standard", label: "Gói Tiêu chuẩn", value: "350000", unit: "month" },
      { id: "tier-wh-premium", label: "Gói Cao cấp", value: "500000", unit: "month" },
    ]);
    toast.success("Đã điền thông tin mẫu — kiểm tra rồi nhấn Lưu!");
  };
  ...
  const performReverse = useCallback(async (lat: number, lng: number) => {
    ...
    try {
      ...
      const { houseNumber, road, district, prov } =
        parseAddress(result.address, result.display_name);
      setFormData((prev) => ({
        ...prev,
  ...
  const handleGeocode = async () => {
    ...
    try {
      ...
      if (results.length === 1) applyResult(results[0]);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setCertFiles((prev) => [...prev, ...pdfs.map((f) => ({ name: f.name, size: f.size, file: f }))]);
    e.target.value = "";
  };
  const removeCert = (idx: number) => setCertFiles((prev) => prev.filter((_, i) => i !== idx));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Upload certification PDFs to storage (if any)
      ...
      if (formData.hasCertification && certFiles.length > 0) {
        for (const cf of certFiles) {
          try {
            ...
            console.warn(`[AddWarehouse] Cert upload failed for ${cf.name}:`, err?.message);
          }
        }
      }
      ...
      const newWarehouse: import("../../../types").ColdStorage = {
        ...
        location: {
          address: [formData.houseNumber, formData.address].filter(Boolean).join(" ") || formData.city,
          city: formData.city,
        ...
        hasCertification: formData.hasCertification && certFiles.length > 0,
      ...
      console.log("[AddWarehouse] Submitting warehouse:", newWarehouse.id, newWarehouse.name);
      try {
        await warehousesAPI.create(newWarehouse);
        console.log("[AddWarehouse] Success:", newWarehouse.id);
        toast.success(`Đã gửi kho "${newWarehouse.name}" để chờ duyệt! Nhân viên sẽ xem xét và kích hoạt kho của bạn.`);
        navigate("/warehouse/my-warehouses");
      } catch (err: any) {
        console.error("[AddWarehouse] createWarehouse failed:", err);
        toast.error(`Lỗi tạo kho lạnh: ${err?.message ?? err}`);
        return;
      }
    } catch (err: any) {
      console.error("[AddWarehouse] Unexpected submit error:", err);
      toast.error(`Có lỗi xảy ra khi thêm kho lạnh: ${err?.message ?? err}`);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <h1 className="text-3xl font-bold mb-2">Thêm kho lạnh mới</h1>
              <p style={{ color: "var(--color-text-secondary)" }}>
                Điền thông tin để đăng ký kho lạnh lên nền tảng
              </p>
            </div>
            <button
              type="button"
              onClick={quickFill}
              className="flex items-center gap-2 px-4 py-2 border-2 text-sm transition-colors shrink-0"
              style={{
              ...
              onMouseLeave={e => { e.currentTarget.style.background = "rgba(245,158,11,0.06)"; e.currentTarget.style.color = "var(--color-warning)"; }}
            >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => { updateField("province", v); setLocationConfirmed(false); }}
                  >
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Quận 7"
                    value={formData.city}
                    onChange={(e) => { updateField("city", e.target.value); setLocationConfirmed(false); }}
                    required
                  />
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <div className="w-28 shrink-0">
                  ...
                  <Input
                    ...
                    onChange={(e) => { updateField("houseNumber", e.target.value); setLocationConfirmed(false); }}
                  />
                </div>
                <div className="flex-1">
                  ...
                  <Input
                    ...
                    onChange={(e) => { updateField("address", e.target.value); setLocationConfirmed(false); setSearchResults([]); }}
                  />
                </div>
              </div>
              ...
              {searchResults.length > 1 && (
                <div className="border border-[var(--color-border)]">
                  <div className="px-3 py-2 border-b border-[var(--color-border)] flex items-center justify-between">
                    ...
                    <button type="button" onClick={() => setSearchResults([])} className="text-[var(--color-text-muted)] hover:text-[var(--color-error)] transition-colors">
                      <X className="h-3.5 w-3.5" />
                    </button>
                  </div>
                      ...
                      <li key={r.place_id}>
                        ...
                        >
                          <span className="flex items-start gap-2">
                            ...
                            <span>
                              <span className="text-xs font-medium block">{i + 1}. {r.display_name}</span>
          ...
          <Card className="bento-card p-6">
            ...
            <p className="text-sm mb-2" style={{ color: "var(--color-text-secondary)" }}>
              Mỗi phân khu có nhiệt ��ộ, sức chứa và giá thuê riêng. Thêm ít nhất một phân khu để lưu kho.
            </p>
            ...
            {sections.length === 0 && (
              <div className="border-2 border-dashed border-[var(--color-error)] px-6 py-8 flex flex-col items-center gap-3 my-4" style={{ opacity: 0.8 }}>
                ...
                <p className="text-xs text-center" style={{ color: "var(--color-text-muted)", maxWidth: 360 }}>
                  Ví dụ: Phòng đông lạnh −25°C, Phòng mát 2°C, Khu đóng gói... mỗi phân khu có giá và sức chứa riêng
                </p>
              </div>
            )}
              ...
              {!formData.hasCertification && (
                <div className="flex items-start gap-3 border-l-4 border-[var(--color-warning)] px-4 py-3">
                  ...
                  <div className="text-sm">
                    <p className="font-semibold mb-0.5">Cảnh báo</p>
                    <p style={{ color: "var(--color-text-secondary)" }}>
              ...
              {formData.hasCertification && (
                <div className="space-y-3">
                  <button
                    ...
                    className="w-full border-2 border-dashed border-[var(--color-border)] hover:border-[var(--color-primary)] px-6 py-8 flex flex-col items-center gap-2 transition-colors group"
                  >
                    <Upload className="h-7 w-7 text-[var(--color-text-muted)] group-hover:text-[var(--color-primary)] transition-colors" />
                    <span className="text-sm text-[var(--color-text-secondary)] group-hover:text-[var(--color-primary)] transition-colors">
                      Nhấn để tải lên file chứng chỉ PDF
                    </span>
                  ...
                  {certFiles.length > 0 ? (
                    <div className="space-y-2">
                      {certFiles.map((cf, i) => (
                        <div key={i} className="flex items-center gap-3 border border-[var(--color-border)] px-3 py-2.5">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>{fmtSize(cf.size)}</p>
                          </div>
                          <button type="button" onClick={() => removeCert(i)} className="text-[var(--color-text-muted)] hover:text-[var(--color-error)] transition-colors">
                            <X className="h-4 w-4" />
                          </button>
                        </div>
                      ))}
                    ...
                    <p className="text-xs text-center" style={{ color: "var(--color-text-muted)" }}>Chưa có file nào</p>
                  )}
                </div>
              )}
            </div>
          ...
          {/* ── Actions ───────────────────────────────────────────────────── */}
          <div className="flex gap-4">
            <Button type="button" variant="outline" onClick={() => navigate("/warehouse")} disabled={loading}>
              Hủy
            </Button>
            <Button type="submit" disabled={loading} className="flex-1">
              <Save className="h-4 w-4 mr-2" />
              {loading ? "Đang lưu..." : "Lưu kho lạnh"}
            </Button>
          </div>
        </form>
      </div>
